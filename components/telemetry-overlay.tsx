...
export function TelemetryOverlay({ vramUsage, ttft, tps, queueCount, meshStatus }: TelemetryProps) {
    ...
    <div className="fixed bottom-14 right-6 z-40 font-mono text-[9px] pointer-events-none">
      ...
      >
        <div className="flex justify-between items-center border-b border-zinc-900 pb-1 mb-2">
          <span className="text-zinc-500 font-black uppercase tracking-tighter">Telemetry HUD</span>
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            ...
            <span className="text-emerald-400 font-bold">{vramUsage || '0.00 GB'}</span>
          </div>
          ...
          {isExpanded && (
            <>
              <div className="flex justify-between border-t border-zinc-900 pt-1">
                ...
                <span className="text-zinc-300">{ttft > 0 ? `${ttft}ms` : '---'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-600 uppercase">Speed:</span>
                <span className="text-zinc-300">{tps > 0 ? `${tps.toFixed(1)} t/s` : '---'}</span>
              </div>
              <div className="flex justify-between border-t border-zinc-900 pt-1">
                <span className="text-zinc-600 uppercase">Mesh:</span>
                <span className={meshStatus === 'CONNECTED' ? 'text-emerald-500 font-bold' : 'text-zinc-500 font-bold'}>{meshStatus}</span>
              </div>
              <div className="flex justify-between">
