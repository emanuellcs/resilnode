import { pipeline, env, type ObjectDetectionPipeline, type ImageClassificationPipeline } from '@huggingface/transformers';

// Configure environment for browser-based WASM execution
env.allowLocalModels = false;
env.useBrowserCache = true;

let visionPipeline: ObjectDetectionPipeline | ImageClassificationPipeline | null = null;
...
async function initPipeline() {
  ...
  try {
    ...
    self.postMessage({ type: 'STATUS', message: 'Vision Engine Online (DETR-ResNet-50)' });
  } catch (error) {
    self.postMessage({ type: 'ERROR', message: `Vision Init Failed: ${error instanceof Error ? error.message : 'Unknown error'}` });
  }
}
